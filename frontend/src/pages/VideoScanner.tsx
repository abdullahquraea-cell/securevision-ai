...
function VideoScanner() {
  ...
  const handleFile = async (e: React.ChangeEvent<HTMLInputElement>) => {
    ...
    try {
      const res = await api.post("/videocheck/analyze", form, {
  ...
  return (
    <div>
      <h2 className="page-title">🎬 فاحص الفيديو الملغّم</h2>

      <div className="panel">
        <h3>افحص أي فيديو قبل تشغيله</h3>
        <p style={{ color: "#64748b", marginTop: 0 }}>
          يكشف الكود المدسوس، الملفات التنفيذية المخفية (EXE)، الامتدادات المزدوجة الخطيرة، والفيديو المزيّف — بلا تشغيل.
        </p>
        ...
        <label style={{
          ...
          {loading ? "⏳ جارٍ الفحص..." : "📤 اختر فيديو للفحص"}
